-- Re-run full migration with function drop to change return type
begin;
...
-- Ensure domain is unique where present (case-insensitive)
...
-- 1d) View that expands searchable fields
...
-- Drop existing to allow return signature change
...
as $$
  ...
  -- Popular fallback when q is short
  ...
  union all
  -- Fuzzy results when q is meaningful
  select name, domain, category, description, logo_url
  from ranked
